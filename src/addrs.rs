//! Candidate address set for BTC collision scanning.
//!
//! All candidates are legacy P2PKH ("1…") addresses — we parse them into their
//! 20-byte Hash160 representation so the worker loop can compare stack keys
//! without formatting Base58 on each check.
...
/// 78 unique legacy P2PKH candidate addresses.
#[allow(dead_code)]   // used by tests only
pub(crate) const CANDIDATE_ADDRS: &[&str] = &[
...
/// Hash160-to-candidate map — stored as `[u8; 20]` keys for zero-cost lookup.
...
/// Read a JSON address list. Accepts either:
///   - a JSON array of strings
///   - a JSON object with an `"addresses"` array
/// Lines after the first `]` are ignored (allows trailing newline).
fn read_addrs_file<P: AsRef<Path>>(p: P) -> Vec<String> {
    ...
    // Try as `Vec<String>` first — that's what our format emits.
